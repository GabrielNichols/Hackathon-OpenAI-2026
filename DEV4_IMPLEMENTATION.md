...
O composition root live já injeta esse adapter no orchestrator do Dev 3, em vez
de `InMemoryRFQExecutionAdapter`. O Dev 3 nunca recebe diretamente
`ProcurementExecutionService`, porque os DTOs dos dois lados têm papéis
distintos. A confirmação autenticada do plano cria a rodada durável e chama o
dispatch manual idempotente; o teste `test_live_dev3_dev4_integration.py`
valida o caminho integrado.

O tool boundary do Dev 3 chama o adapter para criar a rodada:

```python
await execution.create_round(CreateRFQRoundCommand(...))
```

O composition root live faz o dispatch imediatamente após a confirmação do
plano e mantém o restante do port durável disponível para a continuação:
...
Após as respostas dos fornecedores, o `QuoteDecisionPort` conduz as decisões:
...
O Dev 3 não cria `InMemoryExecutionStore` nem importa a implementação concreta;
ele depende somente do adapter e dos ports.
...
Depois do P0 integrado, a ordem recomendada é:
