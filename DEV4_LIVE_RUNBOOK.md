...
O Dev 3 criará a requisição e injetará o adapter disponibilizado em
`app.state.dev3_execution_adapter`. Essa ligação com o orchestrator é um
**gate pós-merge**: antes de integrar os branches, o E2E valida o Dev 4 pelo
mesmo port durável, mas não substitui nem simula o Dev 3. Para operações Dev 4
já mapeadas, use `app.state.execution_port`.
...
Após `create_round`, o Dev 3 chama `send_round`. Isso apenas prepara os links;
não declara entrega.
